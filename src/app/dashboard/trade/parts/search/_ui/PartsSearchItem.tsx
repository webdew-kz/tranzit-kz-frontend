...
const PartsSearchItem = memo(({ partsInitial, rates, loading, setWishlistLength, isContact = true, isWishBtn = true }: PartsSearchItemProps) => {
	...
	useEffect(() => {
		const fetchIsInWishlist = async () => {
			startTransition(async () => {
				...
				console.log(`Parts ID: ${parts.id}, Is in wishlist: ${isInWishlist}`);
	...
	const handleToggleWishlist = async () => {
		if (isWishlist) {
			...
			const res = await addToWishlist(parts.id!);
			const current = getWishlist();
			if (!current.includes(parts.id!)) {
				localStorage.setItem("wishlistPartss", JSON.stringify([...current, parts.id!]));
			}
...
export default PartsSearchItem
