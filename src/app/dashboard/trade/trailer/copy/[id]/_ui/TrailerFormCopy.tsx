...
import { copyTrailer, getTrailer } from '../actions';
import { cn } from '@/shared/lib/utils';
import { CityInput } from '@/shared/components/widgets/CityInput';

interface ITrailerFormCopyProps {
	id: string
}

export default function TrailerFormCopy({ id }: ITrailerFormCopyProps) {
	...
	const onSubmit = async (data: ITrailer) => {
		...
		startTransition(async () => {
			...
			try {
				const res = await copyTrailer(data, oldPhotos)
