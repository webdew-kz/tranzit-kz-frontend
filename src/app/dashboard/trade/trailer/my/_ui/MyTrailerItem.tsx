...
import { ArrowBigDown, ArrowBigUp, ChevronDown, Copy, Eye, RefreshCcw, SquarePen, X, ZoomIn } from 'lucide-react'
...
const MyTrailerItem = memo(({ trailerInitial, selected, onToggle, setTrailers, loading }: MyTrailerItemProps) => {
	...
	const handleActivate = async (id: string) => {
		...
		startTransition(async () => {
			...
			try {
				...
				setTrailers((prev) => {
					const prevTrailers = prev.filter(trailer => trailer.id !== id)

					return [res.trailer, ...prevTrailers]
				});
				...
				<div className=" flex flex-col md:flex-row md:items-center justify-between">
					...
					<div className=" flex gap-2 w-full lg:w-auto">
						...
						<Button
							...
							onClick={() => handleArchivate(trailer.id!)}
							disabled={pending}
						>
