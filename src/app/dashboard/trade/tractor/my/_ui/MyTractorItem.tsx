...
import { ArrowBigDown, ArrowBigUp, BanknoteArrowUp, Box, CalendarDays, ChevronDown, Container, Copy, EllipsisVertical, Eye, HandCoins, MessageCircleMore, Move3d, MoveHorizontal, MoveRight, RefreshCcw, SquarePen, Tractor, Wallet, Weight, X, ZoomIn } from 'lucide-react'
...
const MyTractorItem = memo(({ tractorInitial, selected, onToggle, setTractors, loading }: MyTractorItemProps) => {
	...
	const handleActivate = async (id: string) => {
		...
		startTransition(async () => {
			...
			try {
				...
				setTractors((prev) => {
					const prevTractors = prev.filter(tractor => tractor.id !== id)

					return [res.tractor, ...prevTractors]
				});
				...
				<div className=" flex flex-col md:flex-row md:items-center justify-between">
					...
					<div className=" flex gap-2 w-full lg:w-auto">
						...
						<Button
							...
							onClick={() => handleArchivate(tractor.id!)}
							disabled={pending}
						>
