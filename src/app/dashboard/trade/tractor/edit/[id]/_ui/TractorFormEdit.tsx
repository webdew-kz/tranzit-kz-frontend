...
import { editTractor, getTractor } from '../actions';
import { cn } from '@/shared/lib/utils';
import { CityInput } from '@/shared/components/widgets/CityInput';



export default function TractorFormEdit({ id }: { id: string }) {
	...
	const onSubmit = async (data: ITractor) => {
		...
		startTransition(async () => {
			...
			try {
				const res = await editTractor(data, id!, oldPhotos)
	...
	return (
		<Card className="w-full p-3 md:p-5 gap-3 md:gap-5">
			<CardHeader className='px-0'>
				<CardTitle className='text-xl text-center'>Редактировать тягач</CardTitle>
			</CardHeader>
