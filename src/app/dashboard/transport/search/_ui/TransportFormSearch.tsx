...
import { LoadingTypeEnum, PaymentMethodEnum, PaymentOtherEnum, PaymentPeriodEnum, TermsEnum, TermsPalletsTypeEnum, TruckTypeEnum } from '@/shared/types/transport.type';
...
export default function TransportFormSearch() {
	...
	const { setSearchTransports } = useTransportSearchStore()

	// const { transports, setTransports } = useTransportStore()
	...
	const onSubmit = async (data: ITransport) => {
		...
		startTransition(async () => {
			...
			try {
				...
				setSearchTransports(res.transports)
				// setTransports([])
			} catch (error) {
				console.error(error)
	...
	return (
		<Card className="w-full p-3 md:p-5 gap-3 md:gap-5 pb-[60px]">
			<CardHeader className='px-0'>
				<CardTitle className='text-xl text-center'>Поиск транспортов</CardTitle>
			</CardHeader>
			<CardContent className='px-0'>
				...
				>
					...
					{/* <div className="grid grid-cols-2 lg:grid-cols-4 w-full gap-3 md:gap-5 items-start">
						...
						<Input
							type='text'
							placeholder="Наименование транспорта"
							className='text-sm'
							required
							{...form.register('title')}
						/>
								...
								<DialogContent className="flex flex-col max-h-[100vh] max-w-[100vw] bg-background p-0">
									...
									<div className="overflow-y-auto p-6 pt-4 flex-1">
										<div className="grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-5">
											...
											<div className=" flex flex-col gap-4">
												<h2 className='font-bold text-lg text-(--dark-accent)'>Потранспортка</h2>
												<div className=" flex flex-col gap-2">
													...
													<Controller
														...
														render={({ field }) => (
															<DatePicker
																value={field.value}
																onChange={field.onChange}
																placeholder='Дата разтранспортки'
															/>
														)}
													/>
													...
													<Input
														type='text'
														placeholder="Место потранспортки"
														className='text-sm'
														required
													...
													<Input
														type='text'
														placeholder="Место разтранспортки"
														className='text-sm'
														required
					...
					<div className="grid sm:grid-cols-2 md:grid-cols-6  w-full gap-3 md:gap-5 items-start">
						...
						<Button
							...
							disabled={pending}
						>
							{pending ? (<><Loader2 className="animate-spin stroke-accent" /> Найти транспорт</>) : "Найти транспорт"}
						</Button>
					</div>
				</form>
			</CardContent>
