...
export default function TransportFormEdit({ id }: { id?: string }) {

	const [transport, setTransport] = useState<Transport>({} as Transport)
	...
	const onSubmit = async (data: Transport) => {
		...
		startTransition(async () => {
			...
			try {
				const res = await editTransport(data, id!)
				...
				setTransport(res.updatedTransport)
	...
	return (
		<Card className="w-full p-3 md:p-5 gap-3 md:gap-5 pb-[60px]">
			<CardHeader className='px-0'>
				<CardTitle className='text-xl text-center'>Редактировать груз</CardTitle>
			</CardHeader>
