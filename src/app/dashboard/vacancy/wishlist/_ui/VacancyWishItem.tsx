...
interface VacancySearchItemProps {
	vacancy: IVacancy
	setVacancys?: React.Dispatch<React.SetStateAction<IVacancy[]>>
	rates?: any
	loading?: boolean
	setWishlistLength?: React.Dispatch<React.SetStateAction<number>>;
}

const VacancyWishItem = memo(
	({ vacancy, rates, loading, setWishlistLength, setVacancys }: VacancySearchItemProps) => {
		...
		const handleRemoveFromWishlist = async () => {
			...
			if (setVacancys) {
				setVacancys(prev => prev.filter(c => c.id !== vacancy.id!));
			}
...
export default VacancyWishItem
