	...
	return (
		<div className="h-[100vh] sm:h-[100vh] bg-[url(/images/bg-home.jpg)] md:bg-[url(/images/bg-home-full.jpg)] bg-no-repeat bg-cover bg-center flex items-center justify-center">
			...
			<div className="flex flex-col gap-3 text-center">
				<Link href='/policy' className='text-(--dark-accent) hover:text-(--foreground) border-b border-dashed border-(--dark-accent) hover:border-(--foreground)'>Политика конфиденциальности</Link>
				<Link href='/aferta' className='text-(--dark-accent) hover:text-(--foreground) border-b border-dashed border-(--dark-accent) hover:border-(--foreground)'>Договор аферты</Link>
				<Link href='/rekvizit' className='text-(--dark-accent) hover:text-(--foreground) border-b border-dashed border-(--dark-accent) hover:border-(--foreground)'>Реквизиты</Link>
			</div>
		</div>
	)
}
